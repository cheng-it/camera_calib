...
namespace halcon_wapper
{
  ...
  // undistort
  //////////////////////////////
  void save5DistortCoefsCamParamToYAML(const HTuple &hv_CamParam)
  {
	if (std::string(hv_CamParam[0].S().Text()) == "area_scan_polynomial" && hv_CamParam.Length() == 13)
	{
	  ...
	  cv::FileStorage fs;
	  fs = cv::FileStorage("camera_instrinsic.yaml", cv::FileStorage::WRITE);
	  if (fs.isOpened())
	  {
		fs << "M" << M;
		fs << "D" << D;
		fs.release();
		cout << "save camera parameters as camera_instrinsic.yaml.\n";
	  }
	}
	else
  ...
  void calibrateMonoCular(const string &imageFolder, const string &plateDescriptionFile, const string &camParamSaveDir)
  {
	// 变量声明
	...
	HTuple  hv_CameraParameters, hv_CameraPose, hv_CarParamVirtualFixed;
...
//     hv_StartParameters[8] = 480;
	...
	save5DistortCoefsCamParamToYAML(hv_CameraParameters);
  }
  ...
  void cv_FindMarisAndPose(const cv::Mat &cv_Image,
	  const HTuple &hv_CamParam, const HTuple &hv_PlateDescription,
	  std::vector<cv::Point2f> &centers, cv::Mat &R, cv::Mat &t)
  {
	HObject ho_Image;
